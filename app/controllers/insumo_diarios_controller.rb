class InsumoDiariosController < ApplicationController
	...
	def create
		...
		if @insumo.save
			...
			add_breadcrumb "Crear Insumo Diario", crear_insumo_diario_path(@insumo.dia.fecha)
			@title = "Crear Insumo Diario"
			render 'new'
		end
	end
	...
	def update
		...
		if @insumo.update_attributes(params[:insumo_diario])
			...
			add_breadcrumb "Editar Insumo Diario", editar_insumo_diario_path(@insumo.dia.fecha)
			@title = "Editar Insumo Diario"
			render 'edit'
		end
	end
